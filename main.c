...
int heart_pattern[] = {
        0x0c6, 0x1EF, 0x1FF, 0x1FF, 0x1FE, 0x07C, 0x038, 0x010
};
...
int main() {
    ...
    SetConsoleTitle(TEXT("digital-clock")); // 設置窗口標題
    CONSOLE_CURSOR_INFO cursorInfo = {1, FALSE};
    SetConsoleCursorInfo(h, &cursorInfo);   // 設置不顯示光標

    SetConsoleTextAttribute(h, 0x0C);   // 淡紅色字體
    draw_heart(h, (COORD){51, 17}); // 畫愛心
    SetConsoleTextAttribute(h, 0x0F);   // 黑底白字

    for(;;) {
        GetLocalTime(&t);   // 獲取本地時間
